...
<#assign eventIdStr = httpUtil.getParameter(request['attributes']['CURRENT_URL'], 'eventId') />
...
<#if eventIdStr?has_content>
  <#assign numQuestions = questionService.getAllQuestionsForEventCount(eventIdStr) />
  <#assign listOfQuestions = questionService.getAllQuestionsForEvent(eventIdStr,0,numQuestions) />
...
<div id="successdiv" style="display:none;" class="portlet-msg-success">${languageUtil.get(locale, "thank-you-for-your-vote")}&nbsp;<a
	...
	<#list listOfQuestions as questionObj>
		<div id="${questionObj.questionId!0}" class="question-container">
		<script>
questionArray.push( {
...
selector: "#${questionObj.questionId} "
	...
	<blockquote>Please tell us about yourself</blockquote>
...
<a href="" class="btn btn-large btn-primary" title="Send" id="sendbtn"
   onclick="return (confirm('${languageUtil.get(locale, "confirm")}?') && getAndSendAnswers());">${languageUtil.get(locale, "send")}</a>
...
<script type="text/javascript">
	...
	function getAndSendAnswer(questionObj) {
		...
		var answer = getAnswer(questionObj);
	...
	function getAnswer(questionObj) {
